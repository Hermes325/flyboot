...
export default async function sizes(
  ...
  try {
    const responseJSON = await fetch(
      `https://functions.yandexcloud.net/d4etdktavt47h0i8r4u8?articul=${poizonId}&price=${price}&category=${category}`,
      {
        method: 'GET',
        redirect: 'follow',
        headers: {
    ...
    if (Array.isArray(response))
      ...
      throw new Error(`ERROR sizes function resp |${JSON.stringify(response)}|`);
